    ...
    if (onFilteredCountChange) {
      const totalProjectEntriesCount = filteredEntries.reduce((sum, entry) => {
        ...
        if (selectedProjectId === 'all') {
          ...
          return sum + works.filter(w => w && w.projectId === selectedProjectId).length;
        }
      }, 0);
      onFilteredCountChange(totalProjectEntriesCount);
    }
  }, [filteredEntries, selectedProjectId, onFilteredCountChange]);
      ...
      {groupedDates.length === 0 ? (
        ...
        <div className="space-y-3">
          <div className="text-xs text-gray-400 font-semibold flex items-center justify-between">
            <span>SHOWING {groupedDates.length} ACTIVE DAYS SUBMITTED BY YOU</span>
          </div>
                              ...
                              >
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 pb-1.5">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {onDeleteEntry && (isAdmin || entry.userEmail?.toLowerCase() === currentUserEmail?.toLowerCase()) && (
                                      <button
                                        onClick={() => onDeleteEntry(entry.id)}
                                        className="p-1 px-1.5 text-rose-650 hover:text-white hover:bg-rose-600 border border-rose-100/40 rounded transition text-[9px] font-bold flex items-center gap-0.5 cursor-pointer"
