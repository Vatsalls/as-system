...
export default function DSRDashboard({ 
  ...
  const availableLocations = useMemo(() => {
    ...
    projectLocations.forEach((loc) => {
      if (loc.north && loc.north.trim() !== '') locSet.add(loc.north.trim());
      if (loc.west && loc.west.trim() !== '') locSet.add(loc.west.trim());
    });
    ...
    return flattenedWorks.map((work) => {
      ...
      if (mapping) {
        if (regionFilter === 'North' && mapping.north) {
          ...
          // If All regions, assign regionally
  ...
  }, [flattenedWorks, projects, projectLocations, regionFilter]);
  ...
  const northProjectsCount = useMemo(() => {
    return projectLocations.filter(loc => loc.north && loc.north.trim() !== '').length;
  }, [projectLocations]);

  const westProjectsCount = useMemo(() => {
    return projectLocations.filter(loc => loc.west && loc.west.trim() !== '').length;
  }, [projectLocations]);
  ...
  const unworkedProjects = useMemo(() => {
    ...
    return projects.map((p) => {
      // Find all submissions for this project across all entries (whole team history)
      const projectSubmissions = entries.filter(entry => 
        entry.works && entry.works.some(w => w.projectId === p.id)
      );
      ...
      if (projectSubmissions.length > 0) {
        ...
        const sortedSubmissions = projectSubmissions.map(e => e.date).sort((a, b) => b.localeCompare(a));
        lastWorkedDateStr = sortedSubmissions[0];
  ...
  }, [projects, entries, unworkedFilter]);
      ...
      <div className="bg-white p-5 rounded-2xl border border-gray-150 shadow-2xs space-y-4">
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 ${isAdmin ? 'lg:grid-cols-5' : 'lg:grid-cols-4'} gap-4 pt-1`}>
          ...
          <div className="flex flex-col gap-1.5 bg-slate-50/40 p-2.5 rounded-xl border border-gray-100">
            ...
            <div className="relative flex bg-gray-100 p-1 rounded-xl border border-gray-200 select-none h-[30px] items-center">
              <div 
              ...
              <button
                ...
                className={`relative z-10 flex-1 text-center py-0.5 text-[10px] font-black transition-colors ${
                      ...
                      <React.Fragment key={item.id}>
                        ...
                        {isExpanded && (
                          <tr className="bg-slate-50/30">
                            <td colSpan={6} className="px-6 py-3 border-t border-b border-gray-150">
        ...
        {activeTab === 'backlinks' && (() => {
          ...
          const allCategories = [
            ...
            ...customSubmissionTypes.map(t => ({
              name: t.name,
              count: filteredWorks.reduce((sum, w) => sum + (Number(w.customValues?.[t.id]) || 0), 0),
          ...
          const activeColumns = [
            ...
            ...customSubmissionTypes.map(t => ({
              key: `custom_${t.id}`,
              label: t.name,
              total: filteredWorks.reduce((acc, w) => acc + (Number(w.customValues?.[t.id]) || 0), 0)
            }))
          ].filter(col => col.total > 0);

          // 3. Project rows holding columns with non-zero elements
          const projectRows = projects.map(proj => {
            const projWorks = filteredWorks.filter(w => w.projectId === proj.id);
                    ...
                    <table className="w-full text-left text-xs min-w-[700px]">
                      ...
                      <tbody className="divide-y divide-gray-150 font-semibold text-gray-700">
                        ...
                        {projectRows.length === 0 && (
                          <tr>
                            <td colSpan={3 + activeColumns.length + 1} className="px-4 py-10 text-center text-gray-400 font-medium font-mono italic">
